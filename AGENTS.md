...
│   │   ├── tests/
│   │   └── benchmarks/
│   ├── injector/       # 静态 ptrace helper；按 Linux 架构加载并安全调度 Agent
...
7. **发布以真实环境矩阵为准**：本地 native smoke 只验证 C/Python 协议和对象遍历。正式发布前必须通过
   Linux glibc 上 CPython 3.10–3.14 × x86_64/AArch64 的 ptrace attach、超时恢复和内存预算测试。
...
对象图语义时，额外运行 `make build-agent` 与 `make test-native`；修改 `capture/injector/` 时在 Linux
运行 `make test-injector`；修改 artifact writer 时还需运行
